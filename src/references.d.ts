/// <reference path="../node_modules/platypus/dist/platypus.d.ts" />

// ts-bundle-start
/// <reference path="references/references.ts" />
/// <reference path="button/button.ts" />
/// <reference path="toggle/toggle.ts" />
/// <reference path="checkbox/checkbox.ts" />
/// <reference path="radio/radio.ts" />
/// <reference path="progress/progressring/progressring.ts" />
/// <reference path="progress/progressbar/progressbar.ts" />
/// <reference path="drawer/drawer.ts" />
/// <reference path="drawer/drawercontroller.ts" />
/// <reference path="modal/modal.ts" />
/// <reference path="slider/slider.ts" />
/// <reference path="range/range.ts" />
/// <reference path="select/select.ts" />
/// <reference path="input/input.ts" />
/// <reference path="file/file.ts" />
/// <reference path="carousel/carousel.ts" />
/// <reference path="listview/listview.ts" />
// ts-bundle-end
